...
def get_float_num():
    #MAYBE HERE - to make sure the float number is a number first, in this case maybe I won't need the is_number function
    thebool = True # to help with repeating until we get a number
    while thebool == True:
...
# works in the original version of it !
def is_number(s):
    try:
        int(s) or float(s) # Mot sure if this behaves correctly, orig version is only float(s) eg the int(s) part is missing
    except ValueError:
        return False
...
# A: Check my comment in get_float_num() func
